  ...
  try {
    ...
    const fetchGemini = (model: string) => {
      ...
      return fetch("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", {
        ...
        body: JSON.stringify({ model, messages: geminiMessages, stream: true }),
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!RETRYABLE_STATUSES.has(response.status)) {
          return errorResponse(req, `Lean Brain error: ${parseGeminiError(errorBody)}`, response.status, rid);
        }
      ...
      if (fetchError instanceof DOMException && fetchError.name === 'TimeoutError') {
        ...
        return errorResponse(req, 'Lean Brain timed out. Please try again.', 504, rid);
      }
      throw fetchError;
    }
